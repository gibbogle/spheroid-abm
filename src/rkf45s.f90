...
subroutine r4_fehl ( f, neqn, y, t, h, yp, f1, f2, f3, f4, f5, s, icase )
...
!  write(*,'(a,i2,2f8.4)') 'r4_fehl: (a) t: ',icase, t,ch
...
!  stop
  ...
  call f ( t + 3.0E+00 * h / 8.0E+00, f5, f2, icase )
  ...
  call f ( t + 12.0E+00 * h / 13.0E+00, f5, f3, icase )
  ...
  call f ( t + h / 2.0E+00, f1, f5, icase )
!
...
subroutine r4_rkf45 ( f, neqn, y, yp, t, tout, relerr, abserr, flag, icase )
...
!
  ...
  if ( neqn < 1 ) then
    write(*,*) 'Error: r4_rkf45: neqn: ',neqn
    flag = 8
    return
  end if

  if ( relerr < 0.0E+00 ) then
    write(*,*) 'Error: r4_rkf45: relerr: ',relerr
    flag = 8
    return
  end if

  if ( abserr < 0.0E+00 ) then
    write(*,*) 'Error: r4_rkf45: abserr: ',abserr
    flag = 8
    return
  end if

  if ( flag == 0 .or. 8 < flag .or. flag < -2 ) then
    write(*,*) 'Error: r4_rkf45: flag: ',flag
    flag = 8
    return
  end if
...
!
  if ( mflag /= 1 ) then
    ...
    if ( t == tout .and. kflag /= 3 ) then
      write(*,*) 'Error: r4_rkf45: kflag: ',kflag
      flag = 8
      return
    end if
...
!
  if ( mflag == 1 ) then
    ...
    kop = 0
    kop_g(icase) = kop
!    write(*,'(a,i2,f8.4)') 'r4_rkf45: (a) t: ',icase, t
    call f ( t, y, yp, icase )
    nfe = 1
...
!    write(*,'(a,i2,f8.4)') 'r4_rkf45: (b) t: ',icase, t
    call f ( t, y, yp, icase )
    nfe = nfe + 1
    flag = 2
    return
